...
Команда учится генерировать строки точной длины, делать валидные и невалидные данные, проверять формат email и телефона, добавлять цифры и спецсимволы в пароль, создавать списки нужного размера, делать элементы уникальными и собирать красивый профиль в формате `ключ: значение`.
