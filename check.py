...
# Сохраняем путь к папке проекта, где лежит этот файл check.py.
PROJECT_ROOT = Path(__file__).resolve().parent

# Сохраняем имена модулей проекта, которые нужно перечитывать перед тестами.
PROJECT_MODULE_PREFIXES = (
    # Модули процедурной версии нужно сбрасывать перед каждым запуском тестов.
    "procedural_version",
    # Модули ООП-версии тоже нужно сбрасывать перед каждым запуском тестов.
    "oop_version",
)
...
# Объявляем функцию, которая очищает кеш перед запуском тестов.
def clear_test_cache():
    # Просим Python заново проверить, какие файлы модулей есть на диске.
    importlib.invalidate_caches()
    # Проходим по копии списка уже импортированных модулей.
    for module_name in list(sys.modules):
        # Считаем, что текущий модуль пока не относится к очищаемым.
        should_remove = False
        # Проходим по префиксам модулей проекта.
        for prefix in PROJECT_MODULE_PREFIXES:
            # Проверяем точное имя модуля или вложенный модуль через точку.
            if module_name == prefix or module_name.startswith(prefix + "."):
                # Помечаем модуль для удаления из кеша импортов.
                should_remove = True
        # Проверяем, нужно ли удалить этот модуль из кеша.
        if should_remove:
            # Удаляем модуль, чтобы следующий импорт прочитал файл заново.
            del sys.modules[module_name]
    # Ищем все папки __pycache__ внутри проекта.
    for cache_dir in PROJECT_ROOT.rglob("__pycache__"):
        # Удаляем папку с байт-кодом Python, если она есть.
