...
def show_failure_hints(result):
    ...
    if not failed_tests:
        ...
        return
    # Печатаем заголовок раздела с понятными подсказками.
    ...
    for test, details in failed_tests:
        ...
        # Печатаем имя теста, который нужно исправить.
        print(f"- {test_name}:")
        # Проверяем, не вернула ли функция None из-за оставшегося pass.
        if "None" in details:
            # Печатаем самую частую причину None в учебном задании.
            print("  Функция вернула None. Проверь, не остался ли внутри pass вместо return.")
        ...
        for suffix, hint in HINTS:
            ...
            if test_name.endswith(suffix):
                # Печатаем найденную подсказку.
                print(f"  {hint}")
                # Останавливаем поиск после первой подходящей подсказки.
                break
        # Выполняем этот блок, если подходящей подсказки не нашли.
        else:
            # Просим прочитать текст ошибки unittest и сравнить результат с заданием.
            print("  Прочитай текст ошибки выше и сравни результат с инструкцией в функции.")
